...
def discover_with_sindy(params: PendulumParams, seeds=6, T=6.0, dt=0.01):
    ...
    with open(ART/"sindy_model.txt", "w") as f:
...
def main():
    ...
    sindy_model = discover_with_sindy(params, seeds=8, T=T, dt=dt)
    ...
    # Evaluate initial design grid
    grid = np.linspace(0.05, 2.5, 20)   # radians
    scores = [discrepancy_objective(th, rival_sim, T=T, dt=dt) for th in tqdm(grid)]
    np.save(ART/"initial_thetas.npy", grid); np.save(ART/"initial_scores.npy", scores)

    # Suggest next θ0 via BoTorch qEI over GP surrogate
    ...
    # 3) Validate: simulate truth (pendulum) vs both models at θ0*
    ...
    with open(ART/"summary.json", "w") as f: json.dump(out, f, indent=2)
